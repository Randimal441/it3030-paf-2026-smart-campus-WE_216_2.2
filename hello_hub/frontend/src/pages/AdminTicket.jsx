...
const FILTER_LABELS = ["All", "Open", "In Progress", "Resolved", "Closed", "Reject"];
...
export default function AdminTicket() {
  ...
  const fetchAdminData = useCallback(async () => {
    ...
    try {
      ...
      const mappedTickets = Array.isArray(ticketResponse.data)
        ? ticketResponse.data.map(mapTicket)
        : [];
      const incomingTechnicians = Array.isArray(technicianResponse.data)
      ...
      setSelectedTechnicianByTicket((previous) => {
        ...
        mappedTickets.forEach((ticket) => {
          if (ticket.assignedTechnicianEmail) {
            next[ticket.id] = ticket.assignedTechnicianEmail;
          }
        });
  ...
  const handleTechnicianSelection = useCallback((ticketId, technicianEmail) => {
    setSelectedTechnicianByTicket((previous) => ({
      ...previous,
      [ticketId]: technicianEmail,
    }));
  }, []);

  const handleAssignTechnician = useCallback(
    async (ticket) => {
      const technicianEmail = selectedTechnicianByTicket[ticket.id];

      if (!technicianEmail) {
        setActionMessage("Please select a technician before assigning.");
        return;
      }
      ...
      try {
        await api.post("/api/tickets/assign", {
          ticketId: ticket.id,
          technicianEmail,
        });
        ...
        setActionMessage(
          requestError?.response?.data?.message || "Failed to assign technician. Please try again."
        );
      } finally {
        setAssigningTicketId("");
      }
    },
    [fetchAdminData, selectedTechnicianByTicket]
  );

  const openRejectModal = useCallback((ticket) => {
    setSelectedTicketForReject(ticket);
    setRejectReason("");
    setRejectError("");
  ...
  const closeRejectModal = useCallback(() => {
    if (rejectingTicketId) {
      return;
    }
    setIsRejectModalOpen(false);
    setSelectedTicketForReject(null);
    setRejectReason("");
    setRejectError("");
  }, [rejectingTicketId]);

  const submitRejectTicket = useCallback(
    async (event) => {
      ...
      if (!selectedTicketForReject?.id) {
        setRejectError("Ticket is not selected.");
        return;
      }

      setRejectError("");
      setActionMessage("");
      setRejectingTicketId(selectedTicketForReject.id);

      try {
        await api.post(`/api/tickets/reject/${selectedTicketForReject.id}`, { reason });
        setActionMessage("Ticket rejected successfully.");
        setIsRejectModalOpen(false);
        setSelectedTicketForReject(null);
        setRejectReason("");
        await fetchAdminData();
        ...
        setRejectError(
          requestError?.response?.data?.message || "Failed to reject ticket. Please try again."
        );
      } finally {
        setRejectingTicketId("");
      }
    },
    [fetchAdminData, rejectReason, selectedTicketForReject]
  );

  const handleCloseTicket = useCallback(
    async (ticket) => {
      if (!ticket?.id) {
        setActionMessage("Ticket is not selected.");
        return;
      }
      ...
      try {
        await api.patch(`/api/tickets/close/${ticket.id}`);
        setActionMessage("Ticket closed successfully.");
        await fetchAdminData();
      } catch (requestError) {
        setActionMessage(
          requestError?.response?.data?.message || "Failed to close ticket. Please try again."
        );
      } finally {
        setClosingTicketId("");
      }
  ...
  const canAssignTicket = (status) => status === "Open" || status === "In Progress";
  const canRejectTicket = (status) => status === "Open";
  const canCloseTicket = (status) => status === "Resolved";
    ...
    <div className="tickets-page">
      <section className="tickets-hero">
        ...
        <div className="tickets-hero-content">
          <h1>Admin Ticket Command Center</h1>
          <p>
            Review all campus incidents, assign technicians quickly, and keep ticket workflows
      ...
      <section className="tickets-content">
        <aside className="tickets-sidebar" aria-label="Ticket filters">
          <button type="button" className="raise-ticket-btn" onClick={fetchAdminData}>
          ...
          <header className="tickets-main-header">
            <div>
              ...
              <span>
                {loading
                  ? "Loading..."
                  : `${filteredTickets.length} shown • ${technicians.length} technician(s)`}
              </span>
            </div>
          </header>
                ...
                >
                  ...
                  <div className="ticket-actions-row">
                    <div className="ticket-assignee">
                      Assigned: {ticket.assignedTechnicianName || "Unassigned"}
                    </div>
                    ...
                    <div className="ticket-assign-controls" onClick={(event) => event.stopPropagation()}>
                      <select
                        ...
                        onChange={(event) =>
                          handleTechnicianSelection(ticket.id, event.target.value)
                        }
                        disabled={!canAssignTicket(ticket.status) || technicians.length === 0}
                      >
                      ...
                      >
                        {assigningTicketId === ticket.id ? "Assigning..." : "Assign Technician"}
                      </button>

                      {canRejectTicket(ticket.status) && (
                        <button
                          type="button"
                          className="btn btn-secondary assign-tech-btn"
                          disabled={rejectingTicketId === ticket.id}
                          onClick={() => openRejectModal(ticket)}
                        >
                          {rejectingTicketId === ticket.id ? "Rejecting..." : "Reject Ticket"}
                        </button>
                      )}
      ...
      {isRejectModalOpen && (
        <>
          <div className="modal-overlay" onClick={closeRejectModal} role="presentation"></div>
          <div className="modal-container" role="dialog" aria-modal="true" aria-labelledby="reject-modal-title">
            <div className="modal-header">
              <h2 id="reject-modal-title">Reject Ticket</h2>
              <button
                type="button"
                className="modal-close-btn"
                onClick={closeRejectModal}
                aria-label="Close reject modal"
                disabled={Boolean(rejectingTicketId)}
              >
                x
              </button>
            </div>

            <form onSubmit={submitRejectTicket} className="raise-ticket-form">
              <div className="form-group">
                <label htmlFor="reject-ticket-id">Ticket ID</label>
                <input id="reject-ticket-id" value={selectedTicketForReject?.id || ""} disabled />
              </div>

              <div className="form-group">
                <label htmlFor="reject-reason">
                  Reject Reason <span className="required">*</span>
                </label>
                <textarea
                  id="reject-reason"
                  name="rejectReason"
                  rows="4"
                  placeholder="Write why this ticket is being rejected"
                  value={rejectReason}
                  onChange={(event) => {
                  ...
                  className={rejectError ? "input-error" : ""}
                  disabled={Boolean(rejectingTicketId)}
                ></textarea>
                {rejectError && <span className="error-message">{rejectError}</span>}
              </div>
              ...
              <div className="modal-footer">
                ...
                <button type="submit" className="btn btn-primary" disabled={Boolean(rejectingTicketId)}>
                  {rejectingTicketId ? "Submitting..." : "Submit Reject"}
                </button>
              </div>
            </form>
          </div>
      ...
      {isDetailsModalOpen && selectedTicketForDetails && (
        <>
          ...
          <div className="modal-container" role="dialog" aria-modal="true" aria-labelledby="details-modal-title">
            <div className="modal-header">
            ...
            <div className="raise-ticket-form">
              ...
              <div className="form-row">
                <div className="form-group">
                  ...
                  <input value={selectedTicketForDetails.role} disabled />
                </div>
                <div className="form-group">
